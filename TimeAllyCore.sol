...
contract TimeAllyCore {
    ...
    public {
        ...
        lastMonthlyHandler = eraswapToken.mou();
        owner = msg.sender;
    }
    ...
    returns(bool)
    {
        require(eraswapToken.mou().sub(lastMonthlyHandler) > 30 days);
        require(eraswapToken.mou().sub(nrtManager.lastNRTRelease()) < 30 days);
        paused = true;
        if (step == 0) {
        ...
        if (remaining == 0) {
            ...
            if (step == 5) {
                step = 0;
                lastMonthlyHandler = lastMonthlyHandler.add(1 minutes);
                paused = false;
            }
        }
        return true;
